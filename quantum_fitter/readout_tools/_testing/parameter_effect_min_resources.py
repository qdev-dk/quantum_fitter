...
for i in tqdm(set_list):
    rd._min_resources = i
    rd.do_fit()
    rd.set_plot_dir(i, param_name='Min_resources (number of single shot)', score_name=None)

    rd.plot_cv_iterations(title=f"{rd._get_file_name_from_path(rd._filePath)}, Min_resources: {i}")
    plt.show()
